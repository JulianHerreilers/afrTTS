...
class Config:
        ...
        n_layers = 2
        lr = 0.001
        graphemes = ['<pad>', '</s>', "'", 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z', 'á', 'ä', 'è', 'é', 'ê', 'ë', 'í', 'ï', 'ó', 'ô', 'ö', 'ú', 'û']
        phonemes = ['<pad>', '<s>', '</s>', '2:', '9', '9y', '@', '@i', '@u', 'A:', 'E', 'N', 'O', 'Of', 'S', 'Z', 'a', 'b', 'd', 'e', 'f', 'g', 'h_', 'i', 'i@', 'j', 'k', 'l', 'm', 'n', 'p', 'r', 's', 't', 'u', 'u@', 'v', 'w', 'x', 'y', 'z', '{']   
...
def g2p_model_init(model_name):
    cfg = cfg_init()
    model_name = model_name.replace("G2P/best_models/","")
    cfg.embed_dim = int(model_name[model_name.index("e")+1:model_name.index("h")])
    cfg.hidden_dim = int(model_name[model_name.index("h")+1:model_name.index("n")])
    cfg.n_layers = int(model_name[model_name.index("n")+1:model_name.index("d")])
    cfg.dropout = float(model_name[model_name.index("d")+1:model_name.index("d")+4])
